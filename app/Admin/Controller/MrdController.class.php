...
require_once './ThinkPHP/Library/Vendor/mpdf/mpdf.php';



class MrdController extends AuthController {
    /*
     * 用户管理
     */
...
//        dump($map);
        ...
        // 普通销售
        $group_map["group_id"] = 3;
        $admin_ids = M("auth_group_access")->where($group_map)->getField("uid",true);
        if($admin_ids){
            $admin_map["admin_id"] = array("in",$admin_ids);
            $admins = M("admin")->where($admin_map)->select();
        }
        $this->admins = $admins;
        $this->display();
    }
    
    public function op_pdf() {
        $admin_id = session("aid");
        $group_id = M("auth_group_access")->where(array("uid" => $admin_id))->getField("group_id");
        if ($group_id == 3) {   // 普通管理员
            $auth = "common";
        ...
        $members = M("member_list")->where($map)->select();
        
        if($arrdateone&&$arrdatetwo){
            $html = "<h2 style='text-align:center'>".$arrdateone."至".$arrdatetwo."OP表</h2>";
            $pdf_name = $arrdateone."至".$arrdatetwo."OP表";
        }else{
            $html = "<h2 style='text-align:center'>OP表</h2>";
            $pdf_name = "OP表";
        }
        $html .= "<table style='width:100%;font-size=14px;' border='1' cellspacing='0' cellpadding='0'>";
        $html .= "<tr >";
        $html .= "<th style='text-align:left;height:35px;' >客户姓名</th>";
        $html .= "<th style='width:10%;text-align:left'>客户性别</th>";
        $html .= "<th style='text-align:left' >联系方式</th>";
        $html .= "<th style='text-align:left' >客户生日</th>";
        $html .= "<th style='width:8%;text-align:left'>FF</th>";
        $html .= "<th style='text-align:left' >预算</th>";
        $html .= "<th style='text-align:left' >负责人</th>";
        $html .= "</tr>";
        foreach ($members as $member){
            $html .= "<tr >";
            $html .= "<td style='height:30px;'>".$member["member_list_name"]."</td>";
            if($member["member_list_sex"]==1){
                $html .= "<td>男</td>";
            }else{
                $html .= "<td>女</td>";
            }
            $html .= "<td>".$member["member_list_tel"]."</td>";
            $html .= "<td>".date("Y-m-d",$member["member_list_birth"])."</td>";
            if($member["isff"]==1){
                $html .= "<td>是</td>";
            }else{
                $html .= "<td>否</td>";
            }
            $html .= "<td>".$member["bugdet"]."</td>";
            $html .= "<td>".getField($member["admin_id"], "admin", "admin_realname", "admin_id")."</td>";
            $html .= "</tr>";
        }
        $html .= "</table>";
    ...
    public function pc_pdf() {
        ...
        if($arrdateone&&$arrdatetwo){
            $html = "<h2 style='text-align:center'>".$arrdateone."至".$arrdatetwo."PC表</h2>";
            $pdf_name = $arrdateone."至".$arrdatetwo."PC表";
        }else{
            $html = "<h2 style='text-align:center'>PC表</h2>";
            $pdf_name = "PC表";
        }
        $html .= "<table style='width:100%;font-size=14px;' border='1' cellspacing='0' cellpadding='0'>";
        $html .= "<tr >";
        ...
        $html .= "<th style='text-align:left' >联系方式</th>";
        $html .= "<th style='text-align:left' >客户生日</th>";
        $html .= "<th style='width:8%;text-align:left'>N</th>";
        $html .= "<th style='text-align:left' >AMP</th>";
        $html .= "<th style='text-align:left' >负责人</th>";
        $html .= "</tr>";
        foreach ($members as $member){
            $html .= "<tr >";
    ...
    public function ref_pdf() {
        ...
        if($arrdateone&&$arrdatetwo){
            $html = "<h2 style='text-align:center'>".$arrdateone."至".$arrdatetwo."REF表</h2>";
            $pdf_name = $arrdateone."至".$arrdatetwo."REF表";
        }else{
            $html = "<h2 style='text-align:center'>REF表</h2>";
            $pdf_name = "REF表";
        }
        $html .= "<table style='width:100%;font-size=14px;' border='1' cellspacing='0' cellpadding='0'>";
        $html .= "<tr >";
    ...
    public function op_runadd() {
        if (!IS_AJAX) {
            $this->error('提交方式不正确', U('member_list'), 0);
        } else {
            $map["member_list_name"] = I('member_list_name');
            $map["member_list_tel"] = I('member_list_tel');
            ...
            if(M("member_list")->where($map)->find()){
                M("member_list")->where($map)->save($sl_data);
    ...
    public function op_del() {
        ...
        if ($rst !== false) {
            $this->success('客户删除成功', U('op', array('p' => $p)), 1);
        } else {
            $this->error('客户删除失败', U('op', array('p' => $p)), 0);
        }
    }
...
//        dump($map);
    ...
    public function pc_runadd() {
        if (!IS_AJAX) {
            $this->error('提交方式不正确', U('pc'), 0);
        } else {
            $sl_data['member_list_name'] = I('member_list_name');
            $sl_data['member_list_sex'] = I('member_list_sex');
            $sl_data['member_list_tel'] = I('member_list_tel');
            ...
            $sl_data["n"] = I("n");
            $sl_data["amp"] = I("amp");
            $sl_data['admin_id'] = session('uid');
            
            $map["member_list_name"] = I('member_list_name');
            $map["member_list_tel"] = I("member_list_tel");
            if(M("member_list")->where($map)->find()){
                $res = M("member_list")->where($map)->save($sl_data);
            }else{
                $res = M("member_list")->add($sl_data);
                $type_data["type_id"] = 2;
                $type_data["addtime"] = time();
    ...
    public function pc_runedit() {
         if (!IS_AJAX) {
            ...
            $this->success('客户修改成功', U('pc'), 1);
        }
    }
    
    public function pc_del() {
        $p = I('p');
        $rst = M('member_list')->where(array('member_list_id' => I('member_list_id')))->delete();
        if ($rst !== false) {
            $this->success('客户删除成功', U('pc', array('p' => $p)), 1);
        } else {
            $this->error('客户删除失败', U('pc', array('p' => $p)), 0);
        }
    }
...
//            }
    ...
    public function remark() {
        ...
        $res = M("member_list")->where(array('member_list_id' => $data['id']))->save($data);
        if ($res !== false) {
